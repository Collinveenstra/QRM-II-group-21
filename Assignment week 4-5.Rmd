...
model <- lm(profit ~ budget, data = movies)
summary(model)
...
Profit = -1,76x10^7 + 3,547 x Budget. If a movie had a budget of 0, expected profits would be about -17,6 million euros. For every 1 euro increase in budget, expected profits rises by 2,55 euro on average.
...
The p value is much larger than 0.05, so we fail to reject the null hypothesis at the 5% significance level. Thus, there is no statistically significant evidence that the release month affects movie revenues, keeping other variables constant.
